...
## Maven
- GroupId: `computer.kolektiv.kalendee` (org family `computer.kolektiv.*`)
- Dual repositories: GitHub Packages `https://maven.pkg.github.com/KolektivComputer/kalendee` **and** Yuri Capital (`maven-releases` / `maven-snapshots`) when credentials are present
...
Kotlin packages may still use `dev.kolektiv.kalendee.*` until a follow-up; Maven group is `computer.kolektiv.kalendee`.
